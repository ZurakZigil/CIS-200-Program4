...
public class Address
{
    ...
    // Precondition:  None
    // Postcondition: A String with the zips' data has been returned with leading zeros
    public String ZipToString()
    {
        string NL = Environment.NewLine; // NewLine shortcut
        string result;                   // Builds formatted string
        string maxZip = Convert.ToString(MAX_ZIP); // used to calculat elength of zip limit
        string minZip = Convert.ToString(MIN_ZIP); // used to calculat elength of zip limit

        result = Convert.ToString(Zip); //converting for using length

        // if the zip is shorter then the max, it will add the appropriate amount of leading zeros
        for(int i=maxZip.Length; i>=minZip.Length; i--)
        {
            if(maxZip.Length > result.Length) // if not maximum length
            {
                result = result.PadLeft(i, '0'); // add a leading 0
                return result;
            }
        }
        return result;
